...
const { note, keys, modal, loading, properties } = toRefs(state)
const modalType = {
...
const checkNote = () => {
    const check = properties.value.required.every((prop) => {
        return note.value.content[prop] && !html.test(note.value.content[prop])
    })
    if (!check) {
...
const editNote = async () => {
    ...
    try {
        checkNote()
        await editStoryblokStory(cleanNote(note.value), locale.value)
            .then((res) => {
                note.value = mapNote(res.story)
                modal.value.message = 'Saved'
                modal.value.state = true
                modal.value.type = 'edited'
                emit('edited', note.value)
            })
            .catch(() => {
...
<template>
    <div v-if="note.content">
        <Editor
            ...
            v-model:text="note.content[editor.value]"
            :title="editor.title"
